        ...
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                if restaurantQueried:
                    ...
                    output = "<html><body>"
                    output += "<h1>%s</h1>" % restaurantQueried.name
                    output += "<form method='POST' enctype='multipart/form-data' action = '/restaurants/%s/edit' >" % restaurantIDPath
                    output += "<input name = 'newRestaurantName' type='text' placeholder = '%s' >" % restaurantQueried.name
            ...
            if self.path.endswith("/delete"):
                ...
                if restaurantQueried:
                    ...
                    output = "<html><body>"
                    output += "<h1>Are you sure you want to delete %s?</h1>" % restaurantQueried.name
        ...
        try:
            ...
            if self.path.endswith("/edit"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    restaurantQueried = session.query(Restaurant).filter_by(id=restaurantIDPath).one()
                    if restaurantQueried:
                        restaurantQueried.name = messagecontent[0]
                        session.add(restaurantQueried)
                        session.commit()
                        self.send_response(301)
                        self.send_header('Content-type', 'text/html')
                        self.send_header('Location', '/restaurants')
            ...
            if self.path.endswith("/delete"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    restaurantQueried = session.query(Restaurant).filter_by(id=restaurantIDPath).one()
                    if restaurantQueried:
                        session.delete(restaurantQueried)
                        session.commit()
                        self.send_response(301)
                        self.send_header('Content-type', 'text/html')
                        self.send_header('Location', '/restaurants')
